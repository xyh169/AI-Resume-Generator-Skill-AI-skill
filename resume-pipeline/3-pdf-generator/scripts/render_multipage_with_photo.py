...
CSS_PATH = ROOT / "resume-pipeline" / "3-pdf-generator" / "resources" / "template_multipage.css"
...
def render_basic_info_section(items: list[tuple[str, str]], photo_uri: str | None) -> str:
    info_html = "".join(
        f'<div class="basic-info-item"><span class="basic-info-label">{escape(label)}</span><span>{escape(value)}</span></div>'
        for label, value in items
    )
    photo_html = (
        f'<div class="photo-box"><img src="{escape(photo_uri)}" alt="Profile photo"></div>'
        if photo_uri
        else '<div class="photo-slot">照片位</div>'
    )
    return (
        '<div class="section">'
        ...
        f'<div class="basic-info-grid">{info_html}</div>'
        f"{photo_html}"
        "</div>"
        "</div>"
        "</div>"
...
def build_html(
    ...
    contact: str,
    sections: list[Section],
    basic_info_items: list[tuple[str, str]],
    photo_uri: str | None,
    page_break_titles: set[str],
) -> str:
    css_text = CSS_PATH.read_text(encoding="utf-8")
    contact_items = parse_basic_info(contact)
    edu_section = sections[0] if sections else None
    body_sections = sections[1:] if len(sections) > 1 else []

    rendered_sections: list[str] = []
    rendered_sections.append(render_basic_info_section(basic_info_items, photo_uri))
    if edu_section:
        rendered_sections.append(add_optional_section_class(render_education(edu_section), edu_section.title, page_break_titles))
    for section in body_sections:
        section_html = render_data(section) if section.section_type == "data" else render_narrative(section)
...
<div class="a4-page">
  <div class="header">
    <h1>{escape(name)}</h1>
    <p>{render_contact_line(contact_items)}</p>
  </div>
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render a multi-page with-photo resume from refined_resume.md.")
    ...
    parser.add_argument("--basic-info", action="append", default=[], help="Basic info in key=value form. Can be passed multiple times.")
    parser.add_argument("--page-break-before-title", action="append", default=[])
    args = parser.parse_args()

    md_text = args.input.read_text(encoding="utf-8")
    name, contact, sections = parse_resume(md_text)
    basic_info_items = parse_kv_pairs(args.basic_info)
    photo_uri = args.photo.resolve().as_uri() if args.photo else None
    html_text = build_html(
        ...
        basic_info_items,
        photo_uri,
        set(args.page_break_before_title),
    )
